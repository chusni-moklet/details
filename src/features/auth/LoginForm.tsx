"use client";

import Link from "next/link";
import { motion } from "framer-motion";
import { GraduationCap, AlertCircle, LogIn, Shield } from "lucide-react";
import { Button } from "@/components/ui/button";
import { signInWithGoogle } from "@/app/actions/auth";

interface LoginFormProps {
  error?: string;
  redirectTo?: string;
}

const errorMessages: Record<string, string> = {
  oauth_failed:   "Login gagal. Silakan coba lagi.",
  access_denied:  "Akses ditolak. Gunakan email sekolah (@moklet.com atau @smktelkom-mlg.sch.id).",
  invalid_domain: "Email tidak terdaftar. Hanya email sekolah yang diizinkan.",
  no_code:        "Proses autentikasi gagal. Silakan coba lagi.",
};

export function LoginForm({ error }: LoginFormProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="rounded-2xl border border-dark-700/60 bg-dark-800/70 backdrop-blur-xl p-8 shadow-2xl"
    >
      {/* Logo + brand */}
      <div className="text-center mb-8">
        <div className="relative inline-block mb-4">
          <div className="w-16 h-16 rounded-2xl bg-red-500 flex items-center justify-center shadow-xl shadow-red-500/30">
            <GraduationCap className="w-8 h-8 text-white" />
          </div>
          {/* Telkom red glow */}
          <div className="absolute inset-0 rounded-2xl bg-red-500/20 blur-xl -z-10" />
        </div>
        <h1 className="text-2xl font-bold text-white font-display">Selamat Datang</h1>
        <p className="text-gray-400 text-sm mt-1">DETAILS — SMK Telkom Malang</p>
      </div>

      {/* Error */}
      {error && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/20 mb-6"
        >
          <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-red-300">
            {errorMessages[error] ?? "Terjadi kesalahan. Silakan coba lagi."}
          </p>
        </motion.div>
      )}

      {/* Google OAuth */}
      <form action={signInWithGoogle}>
        <Button
          type="submit"
          size="lg"
          variant="white"
          className="w-full gap-3"
        >
          {/* Google G logo */}
          <svg viewBox="0 0 24 24" className="w-5 h-5" aria-hidden="true">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
          </svg>
          Login dengan Google
        </Button>
      </form>

      {/* Domain restriction info */}
      <div className="mt-5 p-4 rounded-xl bg-dark-900/60 border border-dark-700/60">
        <div className="flex items-start gap-2.5">
          <Shield className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
          <p className="text-xs text-gray-400 leading-relaxed">
            Hanya email dengan domain{" "}
            <span className="text-red-400 font-medium">@moklet.com</span> atau{" "}
            <span className="text-red-400 font-medium">@smktelkom-mlg.sch.id</span>{" "}
            yang dapat login ke sistem ini.
          </p>
        </div>
      </div>

      <div className="mt-5 p-3 rounded-xl bg-red-500/5 border border-red-500/15">
        <div className="flex items-start gap-2">
          <LogIn className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
          <p className="text-xs text-gray-400 leading-relaxed">
            Setelah login, profil Anda akan otomatis dibuat. Lengkapi data di dashboard untuk tampil di katalog.
          </p>
        </div>
      </div>

      <div className="mt-6 text-center">
        <Link href="/" className="text-sm text-gray-500 hover:text-red-400 transition-colors">
          ← Kembali ke Beranda
        </Link>
      </div>
    </motion.div>
  );
}
